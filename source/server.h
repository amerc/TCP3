...
const unsigned local_ip_address_lo = 0x0077u;//0/119         1/1
...
void put_ip_packet(unsigned packet[], unsigned total_length, unsigned protocol, unsigned ip_hi, unsigned ip_lo){
	...
	packet[12] = 0x0000;             //checksum   AMER
...
void put_tcp_packet(unsigned tx_packet [], unsigned tx_length){
	...
	//encode TCP header
	tx_packet[payload_start + 0] = tx_source; // Source port 16 bits
	tx_packet[payload_start + 1] = tx_dest; // Destination port
	tx_packet[payload_start + 2] = tx_seq[1]; // Sequence number
	tx_packet[payload_start + 3] = tx_seq[0]; // Sequence number
	tx_packet[payload_start + 4] = tx_ack[1]; // Akcknowledgement number
	tx_packet[payload_start + 5] = tx_ack[0]; // Akcknowledgement number
...
void server()
{
	...
	while(1){
        ...
        put_eth(46); //46
	index = 0;
        tx_packet[0] = 0x0018u;//destination_mac_address_hi;
	tx_packet[1] = 0xf352u;//destination_mac_address_med;
	tx_packet[2] = 0xfbeeu;//destination_mac_address_lo;
	tx_packet[3] = 0x0001u;//local_mac_address_hi;
	tx_packet[4] = 0x0203u;//local_mac_address_med;
	tx_packet[5] = 0x0405u;//local_mac_address_lo;
	tx_packet[6] = 0x0806u;//protocol;
        tx_packet[7] = 0x0001u;//        
        tx_packet[8] = 0x0800u;//
        tx_packet[9] = 0x0604u;//        
        tx_packet[10] = 0x0001u;// 
        tx_packet[11] = 0x0001u;//        
        tx_packet[12] = 0x0203u;//
        tx_packet[13] = 0x0405u;//        
        tx_packet[14] = 0xc0a8u;//
        tx_packet[15] = 0x0077u;//        
        tx_packet[16] = 0x0018u;//
        tx_packet[17] = 0xf352u;//        
        tx_packet[18] = 0xfbeeu;// 
        tx_packet[19] = 0xc0a8u;//        
        tx_packet[20] = 0x0069u;//
        tx_packet[21] = 0xE3B3u;//        
        tx_packet[22] = 0x32B0u;//
	for(bytes=0; bytes<46; bytes+=2){
		put_eth(tx_packet[index]);
		index ++;
	}
